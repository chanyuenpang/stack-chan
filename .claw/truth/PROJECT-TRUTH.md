...
- 对话恢复判据：`state=idle` + `xiaozhi_ready=true` 是设备运行态正常的前提，**但不能作为对话能力恢复的证明**。所有语音/对话恢复结论必须以真实三轮对话或等价的直接语音链路证据为准。
- force OTA 回退闭环完成判定走 app_version + state + xiaozhi_ready + doctor 四条件汇总，但这四条件无法证明真实对话能力；对话恢复需要独立验证。

## 重要索引

- [`features/ota-release-workflow.md`](features/ota-release-workflow.md)：StackChan OTA 发布、远程重启触发升级、实机状态验收链路；OTA 下载成功 ≠ 刷写成功；固件大小一致性作为预发布校验项。
...
- **force OTA 只能短期救援**：`force=1` 适合旧固件回退或救援下发，但必须在升级闭环收口后恢复为 `force=0`；若设备反复重试且下载重新开始，应回到闭环条件与状态一致性排查，不要长期悬挂强制语义。
