...
当前还要额外钉死一个容易被误读的边界：observed mapping seam 只是 truth mapping 通道。当前 adapter result 已收紧为**更真实的中间 truth payload**，它把单一 adapter result 归一化到 `cases[].observed`、`rawResponse`、`providerExecution`、`transcriptAvailability` 这四个同源槽位，但这不代表真实 provider execution、真实 provider call、provider transcript、transcript persistence 或 scoring 已完成。
...
- 当前 adapter result 只是更真实的中间 truth payload；`cases[].observed`、`rawResponse`、`providerExecution`、`transcriptAvailability` 必须明确是 observed mapping seam 的同源视图，共用一条 truth channel，不能各自夸大成独立 provider/runtime capability，也不能被解读成真实 provider call 已发生；
...
- `cases[].observed` / `rawResponse` / `providerExecution` / `transcriptAvailability` 当前虽然同源，但只能回写为 truth mapping seam 已打通，不能回写成 provider integration / 真实 provider call / transcript persistence / scoring 已完成。
