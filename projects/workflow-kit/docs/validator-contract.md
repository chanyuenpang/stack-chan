...
- The current draft runtime artifact contract is intentionally non-executional: top-level `status` is constrained to `draft` or `blocked`, `summary.passed` must remain `false`, and within the provider-backed reserved seam `cases[].status` is intentionally tightened to `blocked | error` only. In that seam, `blocked` binds `preflight-blocked` / guard-blocked semantics, while provider-backed `error` is only a reserved internal propagation slot and must not be read as a real provider execution failure taxonomy. `observed` stays a stub-shaped truth mapping output rather than execution proof.
...
- `adapter-error` is only an internal semantic alias inside that flow; external/runtime-report serialization must still collapse it to `error`.
- Within that contract, `rawResponse` is only the minimal skeleton/truth-payload slot. It is not a full provider payload capture, not a transcript, and not a persistence/retrieval handle. If `rawResponse.summary` is present, it is only a summary slot.
- That same-source propagation is intentionally restricted to the internal lineage `adapter -> runner -> observed mapper -> report`; this tightening prevents downstream layers from inventing alternate provider-backed selection sources, and `rawResponse` / transcript handles no longer permit mixed-source fallback. `report.failureReason` and case-level failure reasons must also remain same-source rather than mixed-source fallback, but none of this means provider-backed execution is available.
- In the currently reachable draft flows, provider-backed reserved-seam case outcomes should remain within `blocked | error`; `blocked` is the honest `preflight-blocked` / guard-blocked path, while the reserved `error` slot exists only for orchestration/runtime skeleton internal error reporting and must not be interpreted as provider execution or a real provider failure taxonomy.
...
- `rawResponse` and transcript references must stay semantically separate: a raw-response slot/handle is not a transcript handle, not a persistence handle, and current draft flows must not use mixed-source fallback between `rawResponse` and `transcriptRef`.
- Provider-backed reserved-slot fields must remain honest in the emitted artifact: `summary.passed` stays `false`; `rawResponse` remains same-source minimal skeleton passthrough rather than proof of a real provider call or full provider payload capture; `providerExecution` stays reserved/unimplemented for true provider-backed execution; `transcriptAvailability` stays non-provider/non-persistent; provider transcript handles stay unavailable; persistence stays reserved/unimplemented; and provider evidence availability must not be set true.
...
- The current failure-taxonomy tightening is therefore limited to the reserved seam only; real provider request/call/execution/transcript/persistence failure taxonomy remains future work.
...
- The default validate family therefore remains static/preflight-only and does not expose runtime/provider-backed execution paths; none of `pnpm validate`, `pnpm validate:all`, `pnpm validate:contracts`, or `pnpm validate:preflight` should be read as opening a runtime/provider-backed path by fallback.
