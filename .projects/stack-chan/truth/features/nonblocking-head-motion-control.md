...
- bus dead 后停止继续普通写入/扭矩写和读移动风暴，恢复探测应按 `flush -> quiet -> ping -> read_pos -> recovered` 分阶段节流推进；只有 `Ping` 成功后才允许 `ReadPos`，避免 bus_dead 状态下无限 `ReadPos` 探测。
...
| `firmware/main/stackchan/motion/motion.cpp` | `Motion::moveWithSpeed()`、`Motion::moveWithSpeedNoHardwareRead()`、`stop()`、`isMoving()`、`hasBusDead()` / `hasHardwareFailure()` 等 motion 聚合接口；`Motion::stop()` 需要区分 bus_dead 时的 no-read abort 与健康路径的 `stopAnimation()` 硬件读。 |
...
3. `_stackchan_update_task()` 后续循环调用 `stackchan_celebrate_tick()` 推进 LED/head frame。
...
- bus dead 后是否停止普通写入/读移动风暴，并按 `stage=flush`、`stage=quiet`、`stage=ping`、`stage=read_pos`、`event=recovered` 的恢复探测逻辑重新同步。
